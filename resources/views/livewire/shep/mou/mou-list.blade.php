  ...
  function confirmDelete(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Livewire.dispatch('deleteRecap', { id: id });
      }
    });
  }
</script>
      ...
      <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-neutral-700">
        ...
        <div>
          <div class="inline-flex gap-x-2">
            <a class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-hidden focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none" href="/create/mou" wire:navigate>
         ...
         <table id="recapTable" class="display w-full">
          ...
          <th scope="col" class="px-6 py-3 text-start border-s border-gray-200 dark:border-neutral-700">
                ...
                @if(auth()->user()->role == 'shep')
                  <th scope="col" class="px-6 py-3 text-start sticky right-0 bg-white dark:bg-neutral-900 z-10">
                            <span class="text-xs font-semibold uppercase text-gray-800 dark:text-neutral-200">
                              Aksi
                            </span>
                          </th>
            ...
            <tbody>
              @foreach($recaps as $recap)
                <tr>
                  ...
                  @if(auth()->user()->role == 'shep')
                          <td class="h-px w-auto whitespace-nowrap px-2 py-2 sticky right-0 bg-white dark:bg-neutral-900 z-10">
                            <div class="flex gap-2">
                                ...
                                <button type="button" onclick="confirmDelete({{ $recap->id }})"
                                  class="py-2 px-3 flex justify-center items-center size-11 text-sm font-medium 
                                        rounded-lg border border-transparent 
