  ...
  function confirmDelete(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Livewire.dispatch('deleteSMK', { id: id });
      }
    });
  }
</script>
      ...
      <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-neutral-700">
        <div>
          ...
          <p class="text-sm text-gray-600 dark:text-neutral-400">
            Daftar SMK yang Terdaftar Dalam Sistem
          </p>
        </div>

        <div>
          <div class="inline-flex gap-x-2">
            <a class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-hidden focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none" href="/create/school" wire:navigate>
            ...
            <tbody>
              @foreach($schools as $school)
                ...
                <td class="h-px w-auto whitespace-nowrap">
                            <div class="px-6 py-2">
                              <span class="text-sm text-gray-800 dark:text-neutral-200">{{  $school->npsn }}</span>
                            </div>
                          </td>
                          <td class="h-px w-auto whitespace-nowrap">
                            <div class="px-6 py-2">
                              <span class="text-sm text-gray-800 dark:text-neutral-200">{{  $school->school_name }}</span>
                            </div>
                          </td>        
                <td class="h-px w-auto whitespace-nowrap">
                            <div class="px-6 py-2">
                              <span class="text-sm text-gray-800 dark:text-neutral-200">{{  $school->mainDealer->md_code }}</span>
                            </div>
                          </td>
                          <td class="h-px w-auto whitespace-nowrap">
                            <div class="px-6 py-2">
                              <span class="text-sm text-gray-800 dark:text-neutral-200">{{  $school->mainDealer->md_name }}</span>
                            </div>
                          </td>
                  ...
                  @if(auth()->user()->role == 'shep')
                          ...
                          <td class="h-px w-auto whitespace-nowrap px-2 py-2">
                            <button type="button" onclick="confirmDelete({{ $school->id }})"
                            class="py-2 px-3 flex justify-center items-center size-11 text-sm font-medium 
                                    rounded-lg border border-transparent 
